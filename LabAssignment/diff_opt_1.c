...
#define F1(x,y,z) F1[((ny)*(nz)*(x))+((nz)*(y))+(z)] //This kind of things are called macros and afect whole the program:
#define F2(x,y,z) F2[((ny)*(nz)*(x))+((nz)*(y))+(z)] //those definitios allow to define a matrix in a diferent way such that
                                                     //in cobination with malloc (down) we reserve some part of the memory
                                                     //and we tell the program how to acces to it (because is linear) in the
                                                     //same way we acces a matrix. (Las matrices empiezan por la z, el ultimo
                                                     //de los corchetes nos da las primeras posiciones de la memoria, mas o menos chekear)
                                                     //thanks to that we can set dinamically the dimensions of the matrix.
...
{
  int t, x, y, z;
  //Again the key is in the loop and how we acces to de different places of the memory (related to the above coments)
  for (t = 0; t < time; ++t) 
  {
    for (x = 0; x < nx; x++)
      for (y = 0; y < ny; y++) 
...
{ 
  ...
  dt = dx*dy*dz / kappa;
  count = 0.01 / dt;
  f_final = (count % 2)? f2 : f1;  //What is that??
