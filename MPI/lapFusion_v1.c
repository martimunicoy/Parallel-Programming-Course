...
void laplace_init(float *in, int ri, int rf, int n)
{
  int i;
  float V;
  const float pi  = 2.0f * asinf(1.0f);
  printf("%d, %d\n", ri, rf);
  memset(in, 0, n*(rf-ri+2)*sizeof(float));
  for (i = ri; i < rf; i++) {
...
int main(int argc, char** argv)
{
  ...
  A    = (float*) malloc( n*n*sizeof(float)/size+2*n );
  temp = (float*) malloc( n*n*sizeof(float)/size+2*n );

  // Set boundary conditions
  laplace_init (A, ri, rf, n);
  laplace_init (temp, ri, rf, n);

  //print_matrix(A, n*n/size+2, file_dir);
